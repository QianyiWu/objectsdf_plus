...
root_dir = "../exps" # path to the experiment results
exp_name = 'objectsdfplus_replica' # experiment name
out_dir = "evaluation/scene_results" # path to save the scene evaluation results
...
for idx, scan in enumerate(scans):
    ...
    files = list(filter(os.path.isfile, glob.glob(os.path.join(cur_root, "plots/*_whole.ply"))))
    ...
    cmd = f"python cull_mesh.py --input_mesh {ply_file} --input_scalemat ../data/replica/Replica/scan{idx}/cameras.npz --traj ../data/replica/Replica/scan{idx}/traj.txt --output_mesh {cull_mesh_out}"
    print(cmd)
    os.system(cmd)

    cmd = f"python eval_recon.py --rec_mesh {cull_mesh_out} --gt_mesh ../data/replica/Replica/cull_GTmesh/{scan}.ply"
    print(cmd)
