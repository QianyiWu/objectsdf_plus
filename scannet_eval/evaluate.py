...
def load_poses(scan_id):
    ...
    # pose_path = os.path.join(f'../data/custom/scan{scan_id}', 'pose')
    poses = []
...
root_dir = "../exps/objectsdfplus_scannet" # path to your experiments
exp_name = "scan"   # experiment name
out_dir ="evaluation/eval_mesh" # output directory for evaluation results
...
for idx, scan in enumerate(scenes):
    ...
    if not os.path.exists(cur_root):
        print('Current experment {} folder is not exist, skiped'.format(cur_root))
        continue
    # use lastest timestamps
    dirs = sorted(os.listdir(cur_root))
    cur_root = os.path.join(cur_root, dirs[-1])
    files = list(filter(os.path.isfile, glob.glob(os.path.join(cur_root, "plots/*_whole.ply")))) # the whole mesh path
    ...
    # cam_file = f"../data/custom/scan{idx}/cameras.npz"
    ...
    # intrinsic_path = os.path.join(f'../data/custom/scan{idx}/intrinsic/intrinsic_color.txt')
    ...
    # gt_mesh = os.path.join("../data/scannet/GTmesh_lowres", f"{scan[5:]}.obj") # a low-res version of mesh for evaluation
...
# average the all_results
mean_dict = {}
for key in all_results[0].keys():
    mean_dict[key] = sum(d[key] for d in all_results) / len(all_results)
print(mean_dict)
